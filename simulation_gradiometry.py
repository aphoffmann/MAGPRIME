...
def createMixingMatrix(seed, axis = 0, index = 0):
    random.seed(seed)
    np.random.seed(seed)

    boom_length = 600-10*index

    "Create Sensors"
    s1 = magpy.Sensor(position=(50,50,300 + boom_length), style_size=1.8)
    s2 = magpy.Sensor(position=(-50,50,300+ boom_length/2), style_size=1.8)
    s = [s1,s2]

    "Create Sources"
    d1 = magpy.current.Loop(current=10, diameter=10, orientation=st.Rotation.random(),  position=(random.randint(-35, 35), random.randint(-35, 35), random.randint(15, 285)))
    d2 = magpy.current.Loop(current=10, diameter=10, orientation=st.Rotation.random(), position=(random.randint(-35, 35), random.randint(-35, 35), random.randint(15, 285)))    
    d3 = magpy.current.Loop(current=10, diameter=10, orientation=st.Rotation.random(), position=(random.randint(-35, 35), random.randint(-35, 35), random.randint(15, 285)))
    d4 = magpy.current.Loop(current=10, diameter=10, orientation=st.Rotation.random(), position=(random.randint(-35, 35), random.randint(-35, 35), random.randint(15, 285))) 
    src = [d1,d2,d3,d4]
...
def run():
    ...
    if os.path.exists("magprime_results_gradiometry.csv"):
        # Read the existing CSV file and get the last seed value
        results = pd.read_csv("magprime_results_gradiometry.csv")
        last_seed = results["seed"].iloc[-1]
    else:
        # Create an empty data frame with columns
        results = pd.DataFrame(columns=['seed', 'boom_length','rmse_ica', 'rmse_mssa', 'rmse_ness', 'rmse_picog', 'rmse_sheinker', 'rmse_ream', 'rmse_ubss', 'rmse_waicup', "rmse_b1", "rmse_b2", 
    ...
    for i in tqdm.tqdm(range(last_seed + 1, 40)):
        for index in range(30):
            ...
            Kx = createMixingMatrix(i, 0, index)
            Ky = createMixingMatrix(i, 1, index)
            Kz = createMixingMatrix(i, 2, index)
            ...
            results.to_csv("magprime_results_gradiometry.csv", index=False) 
