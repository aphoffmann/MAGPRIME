...
def run():
    ...
    for i in tqdm.tqdm(range(last_seed + 1, 1)):
        ...
        "Add Noise"
        Bx[0] += np.random.normal(0, 1, N); Bx[1] += np.random.normal(0, 1, N); 
        By[0] += np.random.normal(0, 1, N); By[1] += np.random.normal(0, 1, N); 
        Bz[0] += np.random.normal(0, 1, N); Bz[1] += np.random.normal(0, 1, N); 
