...
def createMixingMatrix(axis = 0):
    ...
    s2 = magpy.Sensor(position=(250,250,800), style_size=1.8)
    s3 = magpy.Sensor(position=(200,250,10), style_size=1.8)
    s = [s1,s2,s3]
    
    "Create Sources"
    d1 = magpy.current.Loop(current=6.1, diameter=50,  position=(200,200,110))
    d2 = magpy.current.Loop(current=-4.1, diameter=40,  position=(250,250,160))    
    d3 = magpy.current.Loop(current=5.1, diameter=20,  position=(270,270,80))
    d4 = magpy.current.Loop(current=-5.1, diameter=30,  position=(300,300,50)) 
    src = [d1,d2,d3,d4]
