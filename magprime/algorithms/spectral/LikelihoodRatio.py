...
def likelihood_ratio(s, threshold=3, broadband_filt_width=10, output_filt_shape=(1,3)):
    broadband_s = np.zeros(np.shape(s))
    narrowband_s = np.zeros(np.shape(s))
    for i in range(np.shape(s)[1]):
        raw_spectrum = s[:,i]
        broadband_spectrum = uniform_filter1d(raw_spectrum, broadband_filt_width)
    ...
    median_filtered_detections = median_filter(detected_pixels, output_filt_shape)
